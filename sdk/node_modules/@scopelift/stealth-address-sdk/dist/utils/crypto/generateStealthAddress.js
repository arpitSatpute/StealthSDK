import { ProjectivePoint, getPublicKey as getPublicKeySecp256k1, getSharedSecret, utils } from '@noble/secp256k1';
import { bytesToHex, hexToBytes, keccak256, publicKeyToAddress as publicKeyToAddressViem } from 'viem/utils';
 ...
 */
function generateStealthAddress({ stealthMetaAddressURI, schemeId = VALID_SCHEME_ID.SCHEME_ID_1, ephemeralPrivateKey }) {
 ...
 */
function validateStealthMetaAddress({ stealthMetaAddress, schemeId }) {
    handleSchemeId(schemeId);
    ...
    const spendingPublicKey = cleanedStealthMetaAddress.slice(0, singlePublicKeyHexLength);
    const viewingPublicKey = cleanedStealthMetaAddress.length === 132
 ...
 * @returns
 */
function isValidCompressedPublicKey(publicKey) {
 ...
 */
function parseKeysFromStealthMetaAddress({ stealthMetaAddress, schemeId }) {
    handleSchemeId(schemeId);
    // Remove the '0x' prefix
    const cleanedStealthMetaAddress = stealthMetaAddress.slice(2);
    const singlePublicKeyLength = 66; // Length for compressed keys
    const spendingPublicKey = cleanedStealthMetaAddress.slice(0, singlePublicKeyLength);
    const viewingPublicKey = cleanedStealthMetaAddress.length === 132
        ? cleanedStealthMetaAddress.slice(singlePublicKeyLength)
        : spendingPublicKey; // Use the same key for spending and viewing if only one is provided
    return {
        spendingPublicKey: ProjectivePoint.fromHex(spendingPublicKey).toRawBytes(true), // Compressed
 ...
 */
function computeSharedSecret({ ephemeralPrivateKey, recipientViewingPublicKey, schemeId }) {
    handleSchemeId(schemeId);
 ...
 */
function getHashedSharedSecret({ sharedSecret, schemeId }) {
    handleSchemeId(schemeId);
    return keccak256(sharedSecret);
}
function isSchemeId1(schemeId) {
    return schemeId === VALID_SCHEME_ID.SCHEME_ID_1;
}
function handleSchemeId(schemeId) {
 ...
 */
function generatePrivateKey({ ephemeralPrivateKey, schemeId }) {
    handleSchemeId(schemeId);
 ...
 */
function getPublicKey({ privateKey, compressed, schemeId }) {
    handleSchemeId(schemeId);
 ...
 */
function getViewTag({ hashedSharedSecret, schemeId }) {
    handleSchemeId(schemeId);
 ...
 */
function getStealthPublicKey({ spendingPublicKey, hashedSharedSecret, schemeId }) {
    handleSchemeId(schemeId);
 ...
 */
function publicKeyToAddress({ publicKey, schemeId }) {
    handleSchemeId(schemeId);
    // Use viem to convert the public key to an address
    return publicKeyToAddressViem(typeof publicKey !== 'string' ? bytesToHex(publicKey) : publicKey);
}

...
import { checkStealthAddress, getViewTagFromMetadata } from '../../..';
 ...
 */
async function getAnnouncementsForUser({ announcements, spendingPublicKey, viewingPrivateKey, clientParams, excludeList = [], includeList = [] }) {
    const publicClient = handleViemPublicClient(clientParams);
    // Validate excludeList and includeList
    const _excludeList = new Set([...new Set(excludeList).values()].map(address => getAddress(address)));
    const _includeList = new Set([...new Set(includeList).values()].map(address => getAddress(address)));
    const processedAnnouncements = await Promise.allSettled(announcements.map(announcement => processAnnouncement(announcement, publicClient, {
        spendingPublicKey,
        viewingPrivateKey,
        clientParams,
    ...
    const relevantAnnouncements = processedAnnouncements.reduce((acc, result) => result.status === 'fulfilled' && result.value !== null
 ...
 */
export async function processAnnouncement(announcement, publicClient, { spendingPublicKey, viewingPrivateKey, excludeList, includeList }) {
    const { ephemeralPubKey: ephemeralPublicKey, metadata, stealthAddress: userStealthAddress, transactionHash: hash } = announcement;
    ...
    if (!isForUser)
        return null;
    if (!hash)
        throw new TransactionHashRequiredError();
 ...
 */
async function shouldIncludeAnnouncement({ hash, excludeList, includeList, publicClient }) {
    if (excludeList.size === 0 && includeList.size === 0)
        return true; // No filters applied, include announcement
    const from = await getTransactionFrom({ hash, publicClient });
    if (excludeList.has(from))
        return false; // Exclude if `from` is in excludeList
    if (includeList.size > 0 && !includeList.has(from))
 ...
 */
export async function getTransactionFrom({ publicClient, hash }) {
    try {

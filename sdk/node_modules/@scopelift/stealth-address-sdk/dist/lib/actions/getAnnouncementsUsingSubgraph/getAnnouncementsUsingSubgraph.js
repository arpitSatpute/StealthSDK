...
import { convertSubgraphEntityToAnnouncementLog, fetchPages } from './subgraphHelpers';
 ...
 */
async function getAnnouncementsUsingSubgraph({ subgraphUrl, filter = '', pageSize = 1000 }) {
...
`;
    ...
    try {
        for await (const batch of fetchPages({
            ...
            allAnnouncements.push(...batch.map(convertSubgraphEntityToAnnouncementLog));
        }
    }
    catch (error) {
        throw new GetAnnouncementsUsingSubgraphError('Failed to fetch announcements from the subgraph', error);
    }
